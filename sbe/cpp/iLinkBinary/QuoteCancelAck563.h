...
#ifndef _ILINKBINARY_QUOTECANCELACK563_H_
#define _ILINKBINARY_QUOTECANCELACK563_H_
...
public:
    ...
    {
        QuoteCancelAck563 skipper(m_buffer, m_offset,
...
#pragma clang diagnostic ignored "-Wtautological-compare"
#endif
        return m_actingVersion >= cancelledSymbolSinceVersion();
#if defined(__clang__)
#pragma clang diagnostic pop
#endif
    }
...
#pragma clang diagnostic pop
    ...
    std::uint64_t getLocation(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 318, sizeof(char) * static_cast<size_t>(length));
        return length;
    }
...
#pragma clang diagnostic pop
    ...
    SBE_NODISCARD QuoteCxlStatus::Value quoteStatus() const SBE_NOEXCEPT
    {
        std::uint8_t val;
...
#pragma clang diagnostic ignored "-Wtautological-compare"
#endif
        return m_actingVersion >= unsolicitedCancelTypeSinceVersion();
#if defined(__clang__)
#pragma clang diagnostic pop
#endif
    }
...
#pragma clang diagnostic pop
    ...
    #pragma GCC diagnostic pop
            ...
            m_bufferLength = bufferLength;
            GroupSize dimensions(buffer, *pos, bufferLength, actingVersion);
            dimensions.blockLength((std::uint16_t)9);
...
#pragma clang diagnostic pop
    ...
    #pragma GCC diagnostic pop
            ...
            m_bufferLength = bufferLength;
            GroupSize dimensions(buffer, *pos, bufferLength, actingVersion);
            dimensions.blockLength((std::uint16_t)4);
            dimensions.numInGroup((std::uint8_t)count);
            m_index = 0;
            m_count = count;
            m_blockLength = 4;
    ...
    #pragma clang diagnostic ignored "-Wtautological-compare"
    #endif
            return m_actingVersion >= quoteErrorCodeSinceVersion();
    #if defined(__clang__)
    #pragma clang diagnostic pop
    #endif
        }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits>& operator << (
    std::basic_ostream<CharT, Traits>& builder, QuoteCancelAck563 _writer)
{
    QuoteCancelAck563 writer(_writer.m_buffer, _writer.m_offset,
