...
#ifndef _FIX_TRADECONDITION_H_
#define _FIX_TRADECONDITION_H_
...
public:
    ...
    TradeCondition &rawValue(std::uint8_t value)
    {
        std::memcpy(m_buffer + m_offset, &value, sizeof(std::uint8_t));
        return *this;
    }

    static bool openingTrade(const std::uint8_t bits)
    {
        return (bits & (1u << 0u)) != 0;
    }

    static std::uint8_t openingTrade(const std::uint8_t bits, const bool value)
    {
        return value ? static_cast<std::uint8_t>(bits | (1u << 0u)) : static_cast<std::uint8_t>(bits & ~(1u << 0u));
    }

    SBE_NODISCARD bool openingTrade() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset, sizeof(std::uint8_t));
        return ((val) & (1u << 0u)) != 0;
    ...
    TradeCondition &openingTrade(const bool value)
    {
        std::uint8_t bits;
    ...
    static bool cmeGlobexPrice(const std::uint8_t bits)
    {
        return (bits & (1u << 1u)) != 0;
    }

    static std::uint8_t cmeGlobexPrice(const std::uint8_t bits, const bool value)
    {
        return value ? static_cast<std::uint8_t>(bits | (1u << 1u)) : static_cast<std::uint8_t>(bits & ~(1u << 1u));
    }

    SBE_NODISCARD bool cmeGlobexPrice() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset, sizeof(std::uint8_t));
        return ((val) & (1u << 1u)) != 0;
    ...
    TradeCondition &cmeGlobexPrice(const bool value)
    {
        std::uint8_t bits;
        std::memcpy(&bits, m_buffer + m_offset, sizeof(std::uint8_t));
        ...
        std::basic_ostream<CharT, Traits>& builder, TradeCondition writer)
    {
        ...
        if (writer.openingTrade())
        {
            builder << R"("openingTrade")";
            atLeastOne = true;
        }
        if (writer.cmeGlobexPrice())
        {
            if (atLeastOne)
            {
                builder << ",";
            }
            builder << R"("cmeGlobexPrice")";
        }
        builder << ']';
        return builder;
    }
